...
<head>
    ...
    <title>Form Kisi-Kisi untuk AFEL</title>
</head>

<body style="font-family: 'Times New Roman', Times, serif; font-size: 12pt;">
    <h2 style="text-align: center;">Form Kisi-Kisi untuk AFEL</h2>
        ...
        @foreach ($blueprints as $blueprint)
            ...
            <div style="margin-top: 30px;">
                @if ($blueprint->ranah_penilaian == 'Kognitif')
                    @include('contents.pdf.partials.blueprints.kognitif', [
                        'materiBlueprints' => $materiBlueprints,
                    ])
                @elseif ($blueprint->ranah_penilaian == 'Afektif')
                    @include('contents.pdf.partials.blueprints.afektif', [
                        'materiBlueprints' => $materiBlueprints,
                    ])
                @elseif ($blueprint->ranah_penilaian == 'Psikomotorik')
                    @include('contents.pdf.partials.blueprints.psikomotorik', [
                        'materiBlueprints' => $materiBlueprints,
                    ])
                @endif
            </div>
